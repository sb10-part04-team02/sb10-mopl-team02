...
// The Sports DB API 호출 실패 예외. 에러 응답 상태 코드를 details로 보존
// details는 ErrorResponse로 클라이언트에 노출되므로 요청 URI 등 내부 정보는 담지 않는다
// (특히 SportsDB는 API 키가 URI 경로에 포함되므로 URI 노출 금지)
// retryable: 일시적 장애(5xx, 429, IO/타임아웃) 여부로, 호출 측 재시도 판단에 사용
public class SportsDbApiException extends ExternalApiException {

  private final boolean retryable;

  public SportsDbApiException(int statusCode) {
  ...
  public SportsDbApiException(Throwable cause) {
