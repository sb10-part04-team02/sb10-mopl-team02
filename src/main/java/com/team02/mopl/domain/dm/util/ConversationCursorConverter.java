...
// 커서 문자열을 대화 정렬 키 값(Instant)으로 변환하는 유틸.
// 외부 입력(커서) 파싱·검증을 서비스 계층에서 처리하기 위해 분리했다.
public final class ConversationCursorConverter {

  private ConversationCursorConverter() {} // 인스턴스 생성 금지

  // 커서 문자열 -> 정렬 키 값(Instant)
  public static Instant toSortKey(ConversationSortBy sortBy, String cursor) {
