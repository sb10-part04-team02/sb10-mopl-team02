-- discover backfill의 실행 간 진행 상태(커서) 저장.
-- 시간별 backfill이 개봉일 내림차순으로 최신 -> 과거로 내려가는데, 다음에 어디까지 내려갈지를 기록
-- media_type 단위 1행(MOVIE/TV). 현재 TMDB만 backfill하므로 source 컬럼 없이 media_type을 PK로 둔다.
