 ...
 *
 * <p>실제 JobRepository(Flyway V3 메타테이블)와 DB upsert를 거쳐 청크 처리/멱등성/실패 격리/알림 트리거를 검증한다. 외부 API 호출을 배제하기
...
class ContentIngestionJobIntegrationTest extends IntegrationTestSupport {
  ...
  void job_happyPath_persistsContentsWithoutAlert() throws Exception {
    ...
    assertThat(tmdbStep.getExecutionContext().getInt(ContentUpsertItemWriter.CONTEXT_KEY_INSERTED))
        .isEqualTo(2);
  ...
  void job_rerunWithSameData_isIdempotent() throws Exception {
    ...
    assertThat(tmdbStep.getExecutionContext().getInt(ContentUpsertItemWriter.CONTEXT_KEY_INSERTED))
        .isZero();
    assertThat(tmdbStep.getExecutionContext().getInt(ContentUpsertItemWriter.CONTEXT_KEY_UPDATED))
        .isEqualTo(2);
