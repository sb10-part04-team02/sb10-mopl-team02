# load-test - k6 부하테스트 (콘텐츠 조회)

이슈 #322(최초 구성), #388(콘텐츠 목록조회 심화 + 조회 공통 인프라).
콘텐츠 조회 API에 대한 k6 부하테스트 환경. 리뷰/플레이리스트/소셜 등 나머지 조회와
쓰기(mutation) 부하는 별도 이슈에서 다룬다.
...
load-test/
  ...
  scenarios/
    content-browse.js       # 콘텐츠: 목록 커서 순회, 상세, 키워드 검색, SPORT 필터 (이번 이슈에서 심화)
    review-read.js          # 리뷰 조회 (dev 기존 버전, 심화는 후속 이슈)
    playlist-read.js        # 플레이리스트 조회 (dev 기존 버전, 심화는 후속 이슈)
...
> 이 PR의 심화 대상은 `content-browse.js`와 조회 공통 인프라(`lib/*`, `config/*`, `seed/bulk-seed.sql`)다.
> `review-read.js`/`playlist-read.js`는 dev 기존 버전 그대로이며, 새 공통 인프라와 하위호환된다(후속 이슈에서 심화).
...
스키마는 Flyway가 앱 기동 시 생성한다(#378, `src/main/resources/db/migration/`).
...
빈 DB로 조회 부하를 걸면 병목이 안 드러난다. 콘텐츠 조회 시나리오는 `bulk{i}@mopl.test` 대량 계정으로 VU를 분산하므로 bulk 시딩이 사실상 필수다.

> 경고: `seed/bulk-seed.sql`은 로그인 가능한 계정을 수천 개 만든다(비밀번호 평문 `loadtest1234`가 저장소에 공개). 로컬 또는 부하 전용 DB에서만 실행한다. 운영/공유 DB 절대 금지.

```bash
# 계정 2000 + 콘텐츠(MOVIE 500 + SPORT 100). 콘텐츠 조회엔 리뷰가 불필요하므로 reviews_per_content=0.
docker compose --env-file .env exec -T db psql -U "$DB_USERNAME" -d "$DB_NAME" \
  -v user_count=2000 -v content_count=500 -v reviews_per_content=0 -v sport_content_count=100 \
  < load-test/seed/bulk-seed.sql
```

`bulk-seed.sql`은 계정/콘텐츠/리뷰를 한 파일에 심는다. 콘텐츠 조회만 검증한다면 위처럼 `reviews_per_content=0`으로 리뷰 시딩을 끈다(리뷰 조회 부하가 필요해지면 값을 올린다). 스크립트는 멱등이라 재실행해도 중복이 쌓이지 않는다. 소량 계정만 필요하면(3개) `seed-users.js`를 쓴다.
...
한 여정이 k개의 요청을 보내면 서버 실측 RPS는 대략 `RPS * k`가 된다(content-browse는 목록 순회 + 상세 + 검색 + SPORT로 여정당 여러 요청). 절대 수치를 비교할 때 이 배수를 감안한다.
...
콘텐츠 목록/상세/검색은 요청자 기준 필드가 크지 않지만, 한 계정만 반복 조회하면 같은 캐시 라인과 인덱스 경로만 때려 병목이 희석된다. 그래서 `lib/accounts.js`의 `getSession()`이 VU마다 다른 `bulk{i}@mopl.test` 계정으로 로그인해 부하를 흩뿌린다(요청자 기준 필드가 큰 소셜 조회로 인프라를 그대로 재사용하기 위한 공통 설계이기도 하다).

- `ACCOUNTS`는 실행 프로파일의 maxVUs 이상이어야 한다(작으면 계정이 겹치고, setup이 중단한다).
...
## content-browse SPORT 주의

SPORT 수집은 구현돼 있으나 기본 off라, `bulk-seed.sql`의 `sport_content_count` 시딩이 전제다. SPORT 데이터가 없으면 setup이 중단하니 `-e SKIP_SPORT=1`로 SPORT 시나리오를 건너뛸 수 있다.

## 관찰 / 대상 환경

- 부하 중 서버 지표는 Grafana(`http://localhost:3000`, 기본 admin/admin)로 병행 관찰한다. Prometheus가 앱의 `/actuator/prometheus`를 스크레이프한다(#374). CPU/메모리/DB 커넥션풀/GC를 본다.
