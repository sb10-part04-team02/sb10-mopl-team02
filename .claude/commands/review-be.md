...
- 인자로 PR 번호를 전달하면 해당 PR을, 생략하면 현재 브랜치의 변경사항을 리뷰한다.
...
리뷰 결과를 사용자에게 보여주기 **전에** 위 출력 형식 그대로 로컬에 저장한다.

- 경로: `.local/claude/reviews/<YYYYMMDD>-review-pr<PR번호>.md`
  (브랜치 리뷰 시 `<YYYYMMDD>-review-<브랜치slug>.md`)
- 내용: 리뷰 대상(PR 번호 또는 브랜치) / 리뷰 출력 전체(문제점·이유·개선안)
